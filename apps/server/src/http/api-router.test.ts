...
describe("createApiRouter", () => {
  ...
  it("resolves linkable file references in batch", async () => {
    const tmpRoot = await mkdtemp(path.join(os.tmpdir(), "vde-monitor-files-resolve-"));
    try {
      await mkdir(path.join(tmpRoot, "src"), { recursive: true });
      await writeFile(path.join(tmpRoot, ".gitignore"), "build/\n");
      await writeFile(path.join(tmpRoot, "src", "exists.ts"), "export const exists = true;\n");
      await writeFile(path.join(tmpRoot, "src", "alpha.ts"), "export const alpha = true;\n");
      ...
      expect(res.status).toBe(200);
      const data = await res.json();
      expect(data.linkableRawTokens).toEqual(["src/exists.ts:1", "alpha.ts"]);
    } finally {
      await rm(tmpRoot, { recursive: true, force: true });
    }
  });
  ...
  it("returns 400 when resolve references payload is invalid", async () => {
    ...
    try {
      ...
      const res = await api.request("/sessions/pane-1/files/resolve", {
        ...
        body: JSON.stringify({
          references: [{ rawToken: "missing-fields" }],
        }),
      });
